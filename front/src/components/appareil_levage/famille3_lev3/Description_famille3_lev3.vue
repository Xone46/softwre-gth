        ...
        <table>
            ...
            <tr>
                ...
                <td>
                    ...
                    <p>Distance du centre de gravité en (mm) : <input type="text" :value="description.distanceCentreGravite"
                            @input="saisirDistanceCentreGravite($event)"></p>

                    <p>Charge maximale utile (kg) a hauteur de levée maximale : <input type="text" :value="description.chargeMaximalUtileHauteurLeveeMaximale"
                    ...
                    <p>Pour chariots sans marquage (ni CE ni Epsilon) Charge maximale mat incliné et a la hauteur maximale (m) : <input type="text" :value="description.chariotsSansMarquage"
                        ...
                        <p v-for="item in description.mecanismes" :key="item.index">
                            ...
                            <ul v-if="item.status">
                                <li v-for="el in item.tab" :key="el.index">
                                    ...
                                    {{ el.titre }}
                                    <input v-if="el.status && el.content !== undefined" type="text" :value="el.content" @input="saisirContentMecanisme($event, item.index, el.index)">
                                </li>
                            </ul>
                        </p>
                    </div>
                ...
                <td>
                    <div>
                        <p v-for="item in description.sourceEnergie" :key="item.index">
                            <input type="checkbox" @input="saisirSourceEnergie(item.index)" :value="item.status">{{ item.titre}}
                        </p>
                    </div>
                </td>
            </tr>
                ...
                <td>
                    <div>
                        <p v-for="item in description.translation" :key="item.index">
                            <input type="checkbox" @input="saisirTranslation(item.index)" :value="item.status">{{ item.titre}}
                        </p>
                    </div>
                </td>
            </tr>
                ...
                <td>
                    <div>
                        <p v-for="item in description.dispositifsElevation" :key="item.index">
                            <input type="checkbox" @input="saisirDispositifsElevation(item.index)" :value="item.status" >{{ item.titre}}
                            <ul v-if="item.status">
                                <li v-for="el in item.tab" :key="el.index">
                                    <input type="checkbox" @input="saisirSousDispositifsElevation(item.index, el.index)">
                                    {{ el.titre }}
                                    <input v-if="el.status && el.content !== undefined" type="text" :value="el.content" @input="saisirContentDispositifsElevation($event, item.index, el.index)">
                                    <ul v-if="el.status && el.content === undefined">
                                        <li v-for="elem in el.sous_tab" :key="elem.index">
                                            <input type="checkbox" @input="saisirSousSousDispositifsElevation(item.index, el.index, elem.index)">
                                            {{ elem.titre }}
                                            <input v-if="elem.status && elem.content !== undefined" type="text"  @input="saisirContentSousSousDispositifsElevation($event, item.index, el.index, elem.index)" :value="elem.content">
                                        </li>
                                    </ul>
                                </li>
                            </ul>
                        ...
                        <p v-for="item in description.dispositifPrehension" :key="item.index">
                            ...
                            <ul v-if="item.status && item.tab !== undefined">
                                <li v-for="el in item.tab" :key="el.index">
                                    ...
                                    {{ el.titre }}
                                    <input v-if="el.status && el.content !== undefined" type="text" :value="el.content" @input="saisirContentDispositifPrehension($event, item.index, el.index)">
                                </li>
                            </ul>
                        </p>
                    </div>
                ...
                <td>
                    <div>
                        <p v-for="item in description.equipementsInterchangable" :key="item.index">
                            <input type="checkbox" @input="saisirEquipementsInterchangable(item.index)" :value="item.status" >
                            {{ item.titre }}
                            <input v-if="item.status" type="text" :value="item.content" @input="saisirContentEquipementsInterchangable($event, item.index)">
                        </p>
                    </div>
                    <div>
                        <p>Si présence : Identification de l'équipement concerné : <input type="text" v-model="description.mecanismes.siPresence"></p>
                    </div>
                </td>
            </tr>
